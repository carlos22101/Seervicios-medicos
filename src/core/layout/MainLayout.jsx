...
export default function MainLayout({ children }) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white">
      ...
      {/* --- HEADER --- */}
      {/* h-[80px]: Altura fija similar a la imagen */}
      {/* border-b border-gray-200: La línea gris sutil en la parte inferior */}
      <header className="bg-white shadow-sm border-b border-gray-200 h-20 px-8 flex justify-between items-center relative z-20">
        
        {/* IZQUIERDA: LOGO CON LINK */}
        ...
        <div className="relative">
            ...
            {/* Ícono de Usuario (Click para abrir menú) */}
            <button 
                onClick={() => setShowMenu(!showMenu)}
                className="text-blue-900 hover:text-blue-700 transition-colors focus:outline-none flex items-center justify-center"
            >
                {/* SVG idéntico al de tu imagen de referencia (Circular con usuario dentro) */}
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-10 h-10">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M17.982 18.725A7.488 7.488 0 0012 15.75a7.488 7.488 0 00-5.982 2.975m11.963 0a9 9 0 10-11.963 0m11.963 0A8.966 8.966 0 0112 21a8.966 8.966 0 01-5.982-2.275M15 9.75a3 3 0 11-6 0 3 3 0 016 0z" />
                </svg>
            </button>

            {/* MODAL DESPLEGABLE (Cajita flotante) */}
            {showMenu && (
                <>
                    ...
                    {/* Menú visible */}
                    <div className="absolute right-0 mt-3 w-56 bg-white rounded-lg shadow-xl border border-gray-100 z-20 overflow-hidden animate-fadeIn">
                        
                        {/* Info del usuario */}
                        <div className="px-4 py-3 border-b border-gray-100 bg-gray-50">
                            <p className="text-xs text-gray-500 uppercase font-bold tracking-wider">Usuario Conectado</p>
                            <p className="text-sm font-medium text-gray-900 truncate mt-1">
                                {user?.name || user?.email || 'Dr. Usuario Prueba'}
                            </p>
                        </div>
